...
function EditUser() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setError(null);
            const response = await fetch(`https://final-be-production-ddac.up.railway.app/api/auth/user/${id}`, {
                method: "GET",
                headers: {
    ...
    const deleteRentedProduct = async (id?: string) => {
        const res = await fetch(`https://final-be-production-ddac.up.railway.app/api/products/${id}`, {
            method: "DELETE",
            headers: {
